...
const passwordInputClasses =
  'h-auto rounded-md border border-input bg-background px-4 py-2.5 text-sm text-foreground placeholder:text-muted-foreground focus-visible:border-[#1e3a5f] focus-visible:ring-2 focus-visible:ring-[#1e3a5f1f]';
  ...
  return (
    <div className="flex min-h-dvh items-center justify-center bg-background px-6 py-12">
      <div className="w-full max-w-md rounded-lg border border-border/70 bg-card px-8 py-10 shadow-[0_30px_80px_-65px_rgba(15,23,42,0.65)]">
        ...
        {tokenState === 'checking' ? (
          <div className="mt-8 rounded-md border border-border bg-muted/40 px-4 py-4 text-center text-sm text-muted-foreground">
            {t('submitting')}
          </div>
        ) : tokenState === 'invalid' ? (
          <div className="mt-8 space-y-5">
            <div className="rounded-md border border-destructive/20 bg-destructive/5 px-4 py-4 text-sm text-destructive">
              {t('invalidToken')}
            </div>
            <Link
