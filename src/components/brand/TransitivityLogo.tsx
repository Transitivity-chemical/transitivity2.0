...
const sizes = {
  sm: { icon: 24, fontSize: '0.875rem', supSize: '0.5rem', gap: 6 },
  md: { icon: 32, fontSize: '1.125rem', supSize: '0.625rem', gap: 8 },
  lg: { icon: 40, fontSize: '1.5rem', supSize: '0.75rem', gap: 10 },
  xl: { icon: 56, fontSize: '2rem', supSize: '1rem', gap: 12 },
  '2xl': { icon: 80, fontSize: '3rem', supSize: '1.25rem', gap: 16 },
};
...
export function GammaIcon({
  ...
  return (
    <svg
      ...
      className={className}
    >
      {/* γ (lowercase gamma) — smooth bezier traced from gamablue.png */}
      <path
...
export function TransitivityLogo({
  size = 'md',
  showText = true,
  className = '',
  color,
  ...
  return (
    <div className={`flex items-center ${className}`} style={{ gap: s.gap }}>
      <GammaIcon size={s.icon} color={color || 'currentColor'} />
      {showText && (
        <span
          style={{
...
export function GammaIconRound({
  size = 40,
  bgColor = '#1e3a5f',
  iconColor = '#ffffff',
  className,
