...
export const mdGenerateSchema = z
  .object({
    ...
    functional: z
      ...
      .default('PBE'),
    pseudopotential: z.string().min(1).max(20).default('MT'),
    charge: z.number().int().default(0),
    lsd: z.number().int().min(0).max(1).default(1),
    temperature: z.number().positive().default(300),
    maxSteps: z.number().int().positive().default(50000),
    timeStep: z.number().positive().default(5.0),
    latticeA: z.number().positive().default(10.0),
    latticeB: z.number().positive().default(10.0),
    latticeC: z.number().positive().default(10.0),
